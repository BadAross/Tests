...
{
    public partial class My_Tests_Form : Form
    {
        public My_Tests_Form()
        {
            InitializeComponent();
        ...
        private void Show_Information_In_DataGridView()
        {
            ...
            DataSet dataSet = Data_Search_By_Sql_Query.Returns_Data_From_Database_On_Request(@"select
                  id_Test
                , Name_Test
                from Tests
                join Testers on Id_Tester = Tester_Id
                where Id_Tester = " + id_Active_Account + @"
                order by Name_Test ASC");
