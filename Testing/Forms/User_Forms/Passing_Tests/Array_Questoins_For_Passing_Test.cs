...
namespace Testing.Classes.Data
{
    internal class Array_Questoins_For_Passing_Test
    {
        ...
        public readonly uint id_Test;
        public readonly uint test_Time;
        ...
        public Array_Questoins_For_Passing_Test(string name_Test)
        {
            ...
            id_Test = Convert.ToUInt16(test_Questions.Tables[0].Rows[0]["Id_Test"]);
            test_Time = Convert.ToUInt16(test_Questions.Tables[0].Rows[0]["Time_Complete"]);
        }
        ...
        private int[] Composes_Random_Sequence(int number_Random_Elements)
        {
            ...
            {
                response_Options[i] = Returns_Random_Number(number_Random_Elements, response_Options, i);
            }
            return response_Options;
        }

        private int Returns_Random_Number(int number_Random_Elements, int[] response_Options, int i)
        {
            var rnd = new Random();
            int random_Number;
            ...
            while (Checks_There_Number_Array(response_Options, random_Number, i));
            {
                return random_Number;
            }
        }

        private bool Checks_There_Number_Array(int[] response_Options, int random_Number, int i)
        {
            for (int j = 0; j < i; j++)
            {
